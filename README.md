...
| `MoltenSaveImage`         | `[path]`              | Save the current image output to `path`. If omitted, prompts for a destination path. Works with `snacks-gallery.nvim` image windows as well |
| `SaveImage`               | `[path]`              | Compatibility alias for `MoltenSaveImage` |
...
| `g:molten_image_provider`                     | (`"none"`) \| `"image.nvim"` \| `"snacks-gallery.nvim"` \| `"wezterm"` \| | How images are displayed see [Images](#images) for more details |
...
| `g:molten_snacks_gallery_position`            | (`"bottom"`) \| `"right"`                                   | Position of image windows for the `snacks-gallery.nvim` image provider |
...
Molten has three image providers, `image.nvim`, `snacks-gallery.nvim`, or `wezterm`:
...
- `snacks-gallery.nvim` also requires the [snacks.nvim image module](https://github.com/folke/snacks.nvim/blob/main/docs/image.md).
  It prioritizes stability over tight inline integration: textual output stays in Molten's normal
  output window, while images are shown in separate floating windows next to that output. This is
  intended as a more conservative alternative when inline snacks rendering is unstable. Image
  window placement can be controlled with `g:molten_snacks_gallery_position`.
  When using `"right"`, you can optionally override the anchor column with
  `b:molten_snacks_gallery_right_anchor_col` (buffer-local) or
  `g:molten_snacks_gallery_right_anchor_col` (global). If unset, Molten estimates the cell's
